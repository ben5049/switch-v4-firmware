...
static void sja1105_check_status_msg(SJA1105_HandleTypeDef *dev, SJA1105_StatusTypeDef to_check, bool recurse){
    ...
    /* Return immeditely if everything is fine */
    ...
    switch (to_check){
        ...
        case SJA1105_RAM_PARITY_ERROR:
            status = SJA1105_ReInit(dev, sja1105_static_conf, sja1105_static_conf_size);
            if (dev->initialised) error_solved = true;
            break;
...
void switch_thread_entry(uint32_t initial_input){
    ...
    status = SJA1105_Init(&hsja1105, &sja1105_conf, sja1105_ports, &sja1105_callbacks, sja1105_static_conf, sja1105_static_conf_size); sja1105_check_status_msg(&hsja1105, status, true);

    /* Set the speed of the dynamic ports. TODO: This should be after PHY auto-negotiaion */
    status = SJA1105_UpdatePortSpeed(&hsja1105, PORT_88Q2112_PHY0, SJA1105_SPEED_1G); sja1105_check_status_msg(&hsja1105, status, true);
    status = SJA1105_UpdatePortSpeed(&hsja1105, PORT_88Q2112_PHY1, SJA1105_SPEED_1G); sja1105_check_status_msg(&hsja1105, status, true);
    status = SJA1105_UpdatePortSpeed(&hsja1105, PORT_88Q2112_PHY2, SJA1105_SPEED_1G); sja1105_check_status_msg(&hsja1105, status, true);

    while (1){
        tx_thread_sleep_ms(200);

        /* Perform a regular maintenance check */
        status = SJA1105_CheckStatusRegisters(&hsja1105); sja1105_check_status_msg(&hsja1105, status, true);

        /* Read the temperature */
        status = SJA1105_ReadTemperatureX10(&hsja1105, &temp_x10); sja1105_check_status_msg(&hsja1105, status, true);
